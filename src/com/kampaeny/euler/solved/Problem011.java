	...
	private static void _L2RMulti(int[][] regularArray) {
		...
		for (int i = 0; i < length; i++) {
			for (int j = 0; j < length; j++) {
				if ((j + 3) < length) {
					tempValueL2R = regularArray[i][j] * regularArray[i][j + 1] * regularArray[i][j + 2]
							* regularArray[i][j + 3];
					if (finalValueL2R == 0) {
		...
		System.out
				.println("#_L2RMulti(int[][]) = " + finalValueL2R + ", big=" + String.valueOf(finalValueL2R).length());
	...
	private static void _R2LMulti(int[][] regularArray) {
		...
		for (int i = length - 1; i > 0; i--) {
			for (int j = length - 1; j > 0; j--) {
				if ((j - 3) < length && (j - 3 > 0)) {
					tempValueR2L = regularArray[i][j] * regularArray[i][j - 1] * regularArray[i][j - 2]
							* regularArray[i][j - 3];
					if (finalValueR2L == 0) {
		...
		System.out
				.println("#_R2LMulti(int[][]) = " + finalValueR2L + ", big=" + String.valueOf(finalValueR2L).length());
